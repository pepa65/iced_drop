...
impl<'a, Message, Theme, Renderer> Widget<Message, Theme, Renderer> for Droppable<'a, Message, Theme, Renderer>
where
	Message: Clone,
	Renderer: renderer::Renderer,
{
	...
	fn update(&mut self, tree: &mut iced::advanced::widget::Tree, event: &iced::Event, layout: iced::advanced::Layout<'_>, cursor: iced::advanced::mouse::Cursor, _renderer: &Renderer, _clipboard: &mut dyn iced::advanced::Clipboard, shell: &mut iced::advanced::Shell<'_, Message>, _viewport: &iced::Rectangle) {
		...
		if !matches!(state.action, Action::Drag(_, _)) {
			...
			self.content.as_widget_mut().update(&mut tree.children[0], event, layout, cursor, _renderer, _clipboard, shell, _viewport);
			// this should really only be captured if the droppable is nested or it contains some other widget that captures the event
			if shell.is_event_captured() {
				return;
			}
		}

		if let Some(on_drop) = self.on_drop.as_deref() && let iced::Event::Mouse(mouse) = event {
			match mouse {
				mouse::Event::ButtonPressed(btn) => {
					if *btn == mouse::Button::Left && cursor.is_over(layout.bounds()) {
						...
						shell.capture_event();
					} else if *btn == mouse::Button::Right && let Action::Drag(_, _) = state.action {
				...
				&mouse::Event::CursorMoved { mut position } => {
					let should_drag = match state.action {
						Action::Select(start) => {
							...
							let distance = ((position.x - start.x).powi(2) + (position.y - start.y).powi(2)).sqrt();
					...
					if should_drag && let Action::Drag(start, _) = state.action {
						...
						if self.drag_center {
							state.overlay_bounds.x = position.x - state.overlay_bounds.width / 2.;
							state.overlay_bounds.y = position.y - state.overlay_bounds.height / 2.;
						} else {
		...
		if let iced::Event::Window(iced::window::Event::RedrawRequested(_now)) = event {
	...
	fn layout(&mut self, tree: &mut iced::advanced::widget::Tree, renderer: &Renderer, limits: &iced::advanced::layout::Limits) -> iced::advanced::layout::Node {
		...
		let content_node = self.content.as_widget_mut().layout(&mut tree.children[0], renderer, limits);
		...
		if let Some(new_size) = self.drag_size {
			match state.action {
				Action::Drag(_, _) => {
					return iced::advanced::layout::Node::with_children(new_size, content_node.children().to_vec());
				}
	...
	fn draw(&self, tree: &iced::advanced::widget::Tree, renderer: &mut Renderer, theme: &Theme, style: &renderer::Style, layout: iced::advanced::Layout<'_>, cursor: iced::advanced::mouse::Cursor, viewport: &iced::Rectangle) {
		let state: &State = tree.state.downcast_ref::<State>();
		if let Action::Drag(_, _) = state.action && self.drag_hide {
			return;
		}
	...
	fn overlay<'b>(&'b mut self, tree: &'b mut Tree, layout: Layout<'b>, renderer: &Renderer, _viewport: &iced::Rectangle, _translation: Vector) -> Option<overlay::Element<'b, Message, Theme, Renderer>> {
		let state: &mut State = tree.state.downcast_mut::<State>();
		if self.drag_overlay && let Action::Drag(_, _) = state.action {
		...
		self.content.as_widget_mut().overlay(&mut tree.children[0], layout, renderer, _viewport, _translation)
	}

	fn mouse_interaction(&self, tree: &iced::advanced::widget::Tree, layout: iced::advanced::Layout<'_>, cursor: iced::advanced::mouse::Cursor, _viewport: &iced::Rectangle, _renderer: &Renderer) -> iced::advanced::mouse::Interaction {
		let state = tree.state.downcast_ref::<State>();
		if let Action::Drag(_, _) = state.action {
			return mouse::Interaction::Grabbing;
		}
		let child_interact = self.content.as_widget().mouse_interaction(&tree.children[0], layout, cursor, _viewport, _renderer);
...
impl<'a, Message, Theme, Renderer> From<Droppable<'a, Message, Theme, Renderer>> for Element<'a, Message, Theme, Renderer>
where
	Message: 'a + Clone,
	Theme: 'a,
	Renderer: 'a + renderer::Renderer,
{
	fn from(droppable: Droppable<'a, Message, Theme, Renderer>) -> Element<'a, Message, Theme, Renderer> {
...
impl<'a, 'b, Message, Theme, Renderer> overlay::Overlay<Message, Theme, Renderer> for Overlay<'a, 'b, Message, Theme, Renderer>
where
	Renderer: renderer::Renderer,
{
	fn layout(&mut self, renderer: &Renderer, _bounds: Size) -> layout::Node {
		Widget::<Message, Theme, Renderer>::layout(self.content.as_widget_mut(), self.tree, renderer, &layout::Limits::new(Size::ZERO, self.overlay_bounds.size()))
			.move_to(self.overlay_bounds.position())
	}

	fn draw(&self, renderer: &mut Renderer, theme: &Theme, inherited_style: &renderer::Style, layout: Layout<'_>, cursor_position: mouse::Cursor) {
