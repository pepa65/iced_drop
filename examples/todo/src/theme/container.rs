...
pub fn active_slot(_theme: &Theme) -> Style {
	Style {
		...
		border: Border { color: color!(202, 233, 255), radius: 10.0.into(), width: 5.0 },
		..Default::default()
	}
}

pub fn title(_theme: &Theme) -> Style {
	Style { background: Some(color!(131, 122, 117).into()), ..Default::default() }
}
...
pub fn background(_theme: &Theme) -> Style {
	Style { background: Some(color!(96, 91, 86).into()), ..Default::default() }
}
