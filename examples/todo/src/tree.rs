...
pub const NULL_TODO_LOC: TreeLocation = TreeLocation { slot: 0, element: TreeElement::Slot };
...
impl TreeData {
	...
	pub fn view(&self) -> Element<Message> {
		...
		row(children).spacing(10.0).padding(20.0).width(Length::Fill).height(Length::Fill).into()
	}
	...
	pub fn todo_options(&self, t_loc: &TreeLocation) -> Vec<Id> {
		...
		self.slots
			.iter()
			.map(|slot| slot.list.todos.iter().filter_map(|todo| if todo.id != todo_id { Some(todo.id.clone()) } else { None }))
			.flatten()
...
impl Slot {
	...
	pub fn new(list: List) -> Self {
		...
		Self { id: Id::from(c_id.clone()), c_id, list, highlight: false }
	}
	...
	fn view(&self, index: usize) -> Element<Message> {
		container(self.list.view(index))
			.id(self.c_id.clone())
			.style(if self.highlight { theme::container::active_slot } else { container::transparent })
			.width(Length::Fill)
			.height(Length::Fill)
			.padding(3.5)
			.into()
...
impl ElementAdder {
	pub fn new(id: usize) -> Self {
		Self { id: iced::widget::text_input::Id::new(format!("todo_adder_{}", id)), text: String::new() }
	}
...
impl List {
	...
	fn view(&self, slot_index: usize) -> Element<Message> {
		let name = text(self.title.clone()).size(20).style(theme::text::list_name);
		let location = TreeLocation::new(slot_index, TreeElement::List);
		let todos = column(self.todos.iter().enumerate().map(|(i, todo)| todo.view(TreeLocation::new(slot_index, TreeElement::Todo(i)))))
			.spacing(10.0)
			.width(Length::Fill)
		...
		let content = container(column![name, todos].spacing(20.0))
			...
			.height(Length::Shrink)
			.padding(10.0)
			.style(if self.highlight { theme::container::active_list } else { theme::container::list });
		droppable(content)
			.id(self.id.clone())
...
impl Todo {
	...
	fn view(&self, location: TreeLocation) -> Element<Message> {
		let txt = text(&self.content).size(15).style(theme::text::todo).align_y(Center);
		let content = container(txt)
			.align_y(alignment::Vertical::Center)
			.padding(10.0)
			.width(Length::Fill)
			.height(Length::Shrink)
			.style(if self.highlight { theme::container::active_todo } else { theme::container::todo });
