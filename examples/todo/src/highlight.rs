...
pub fn dragged(info: &Highlight, loc: TreeLocation, bounds: Rectangle) -> Highlight {
	Highlight { dragging: Some((loc, bounds)), ..info.clone() }
}
...
pub fn zones_found(info: &Highlight, zones: &Vec<(TreeLocation, Rectangle)>) -> Highlight {
	...
	if let Some((_, bounds)) = info.dragging {
		...
		let valid_zones = if split_zones[0].is_empty() { &split_zones[1] } else { &split_zones[0] };
...
pub fn should_update_droppable(old_info: &Highlight, new_info: &Highlight, loc: &TreeLocation) -> bool {
...
fn bigggest_intersect_area<'a>(zones: &'a Vec<(TreeLocation, Rectangle)>, droppable: &Rectangle) -> Option<(&'a TreeLocation, f32)> {
	zones
		.iter()
		.map(|(id, rect)| (id, rect.intersection(&droppable).unwrap_or(Rectangle::default())))
