...
fn main() -> iced::Result {
	iced::application(TodoBoard::title, TodoBoard::update, TodoBoard::view).theme(TodoBoard::theme).run()
}
...
impl TodoBoard {
	...
	fn view(&self) -> Element<'_, Message> {
		...
		container(column![header, self.tree.view()].height(Length::Fill).width(Length::Fill))
			.width(Length::Fill)
			.height(Length::Fill)
	...
	fn update(&mut self, message: Message) -> Task<Message> {
		match message {
			...
			Message::DragTodo(t_loc, __, t_bounds) => {
				let new_highlight = highlight::dragged(&self.todos_highlight, t_loc.clone(), t_bounds);
			...
			Message::HandleTodoZones(zones) => {
				let new_highlight = highlight::zones_found(&self.todos_highlight, &map_zones(&self.tree, zones));
				zone_update(&self.todos_highlight, &new_highlight).update(&mut self.tree, &self.todos_highlight, &new_highlight);
				self.todos_highlight = new_highlight;
			}
			Message::DropTodo(t_loc, _, _) => {
				if let Some(h_loc) = &self.todos_highlight.hovered {
					match h_loc.element() {
						...
						TreeElement::Todo(_) => todo_dropped_on_todo(&mut self.tree, &t_loc, &h_loc),
						_ => (),
					}
				} else {
			...
			Message::DragList(l_loc, _, l_bounds) => {
				let new_highlight = highlight::dragged(&self.lists_highlight, l_loc.clone(), l_bounds);
				...
				return find_zones(Message::HandleListZones, move |zone_bounds| zone_bounds.intersects(&l_bounds), Some(self.tree.list_options()), None);
			}
			Message::HandleListZones(zones) => {
				let new_info = highlight::zones_found(&self.lists_highlight, &map_zones(&self.tree, zones));
...
fn map_zones(tree: &TreeData, zones: Vec<(Id, Rectangle)>) -> Vec<(TreeLocation, Rectangle)> {
	zones
		.into_iter()
		.filter_map(|(id, rect)| if let Some(loc) = tree.find(&id) { Some((loc, rect)) } else { None })
		.collect()
}
...
fn move_list_to_zone(tree: &mut TreeData, d_loc: &TreeLocation, h_loc: &TreeLocation) -> Task<Message> {
	let l1 = tree.list_mut(d_loc).id();
	let l2 = tree.list_mut(h_loc).id();
	tree.swap_lists(d_loc, h_loc);
	return swap_modify_states(l1, l2, |_old: &DroppableState, new: &DroppableState| new.clone());
}
